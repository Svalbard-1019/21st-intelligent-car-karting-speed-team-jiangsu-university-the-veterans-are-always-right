...
float recode_threshold = 0.3f;         // 路径记录阈值
...
float guandao_debug_speed_requested = 0.0f;
float guandao_debug_speed_command = 0.0f;
uint8 guandao_debug_turn_level = 0u;
...
static uint8 portion3_direct_reverse_active(void)
{
    return portion3_direct_reverse_state != 0u;
}
...
static uint8 portion3_direct_reverse_prepare(guandao_state *route)
{
    ...
    for(int16 i = 0; i < length / 2; i++)
    {
        swap_point = route->recode_map[i];
...
static void portion3_direct_reverse_update(void)
{
    guandao_reverse_speed_plan_t reverse_plan;
    state_t target;
    int16 length;
    int16 search_end;
    int16 best_index;
    ...
    out_v_l = 0.0f;
    out_v_r = 0.0f;
    if(portion3_direct_reverse_state == 2u)
    {
        out_servo = portion3_direct_reverse_steer_cmd;
        daoche_speed = 0.0f;
        daoche_flag = 1;
        conrtol_mode = DAOCHE;
        return;
    }
    if(portion3_direct_reverse_state != 1u) return;
    ...
    if(!stop_requested)
    {
        ...
        best_index = portion3_direct_reverse_route_index;
        best_distance = get_distance(
                portion_3.current_state, portion_3.recode_map[best_index]);
        for(int16 i = portion3_direct_reverse_route_index + 1; i <= search_end; i++)
        {
            float distance = get_distance(
                    portion_3.current_state, portion_3.recode_map[i]);
        ...
        if(best_index > portion3_direct_reverse_route_index)
        {
            portion3_direct_reverse_route_index = best_index;
        }
        while(portion3_direct_reverse_route_index < length - 1
                && get_distance(portion_3.current_state,
                        portion_3.recode_map[portion3_direct_reverse_route_index])
                        <= PORTION3_DIRECT_REVERSE_POINT_DIST)
        {
            portion3_direct_reverse_route_index++;
        }
        portion_3.current_point_index = portion3_direct_reverse_route_index;
        ...
        target_angle = atan2f(
                target.x - portion_3.current_state.x,
        ...
        steer_elapsed_ms = (portion3_direct_reverse_steer_ms == 0u) ? 20u
                : guandao_elapsed_ms(now_ms, portion3_direct_reverse_steer_ms);
        if(steer_elapsed_ms > 100u) steer_elapsed_ms = 20u;
        steer_delta = desired_steer - portion3_direct_reverse_steer_cmd;
        {
            float steer_limit = PORTION3_DIRECT_REVERSE_STEER_RATE
                    * ((float)steer_elapsed_ms / 20.0f);
            Value_Limit_float(&steer_delta, -steer_limit, steer_limit);
        }
        ...
        else
        {
            ...
            out_servo = portion3_direct_reverse_steer_cmd;
            guandao_debug_stop_reason = 14;
            guandao_debug_distance = target_distance;
            guandao_debug_angle_diff = heading_error;
            guandao_debug_dist_final = portion3_direct_reverse_final_dist;
        }
    }

    if(stop_requested)
    {
        portion3_direct_reverse_state = 2;
        daoche_flag = 1;
        conrtol_mode = DAOCHE;
        daoche_speed = 0.0f;
        out_v_l = 0.0f;
...
{
    return portion3_direct_reverse_route_index;
}

float guandao_portion3_reverse_final_distance(void)
{
    return portion3_direct_reverse_final_dist;
}
...
static float guandao_portion1_distance_turn(
        guandao_state *state, int start_index, float window_distance)
{
    float positive_turn = 0.0f;
    float negative_turn = 0.0f;
    float accumulated_distance = 0.0f;
    int16 route_length = guandao_route_length(state);

    if(route_length < 3 || window_distance <= 0.0f) return 0.0f;
    if(start_index < 1) start_index = 1;

    for(int i = start_index;
            i <= route_length - 2 && accumulated_distance < window_distance;
            i++)
    {
        state_t previous = guandao_route_point(state, i - 1);
        state_t current = guandao_route_point(state, i);
        state_t next = guandao_route_point(state, i + 1);
        float yaw_in = guandao_segment_yaw(previous, current);
        float yaw_out = guandao_segment_yaw(current, next);
        float signed_turn = guandao_normalize_angle(yaw_out - yaw_in);

        accumulated_distance += get_distance(current, next);
        if(fabsf(signed_turn) < GUANDAO_P1_TURN_DEADBAND_DEG) continue;
        if(signed_turn > 0.0f) positive_turn += signed_turn;
        else negative_turn -= signed_turn;
    }

    return (positive_turn >= negative_turn) ? positive_turn : -negative_turn;
}
...
//            Buzzer_check(50);
    ...
    else if(upcoming_turn >= GUANDAO_ACCUM_TURN_SHARP_ANGLE)
    {
        accumulated_turn_level = 3u;
    }
    else if(upcoming_turn >= GUANDAO_ACCUM_TURN_MEDIUM_ANGLE)
    {
        accumulated_turn_level = 2u;
    }
    else if(upcoming_turn >= GUANDAO_ACCUM_TURN_SLOW_ANGLE)
    {
        accumulated_turn_level = 1u;
    }
    guandao_debug_turn_level = accumulated_turn_level;
    ...
    if(base_speed >= GUANDAO_FAST_STRAIGHT_SPEED
            && fabsf(upcoming_turn) < GUANDAO_ACCUM_TURN_SLOW_ANGLE
...
//   slip_cheak(&guandao_ecd,target_steering);
   ...
   else if(accumulated_turn_level >= 2u)
   {
   ...
   else if(accumulated_turn_level >= 1u)
   {
   ...
   if(route_setting_choice == 0 || route_setting_choice == 2)
   {
       ...
       uint32 speed_elapsed_ms = (portion1_speed_last_ms == 0u) ? 20u
               : guandao_elapsed_ms(speed_now_ms, portion1_speed_last_ms);
       if(speed_elapsed_ms > 200u) speed_elapsed_ms = 20u;
       if(speed_elapsed_ms >= 1u)
       {
           v_center = guandao_speed_rate_limit(
                   &portion1_speed_planner, v_center,
                   (float)speed_elapsed_ms * 0.001f,
                   GUANDAO_P1_ACCEL_UNITS_PER_S,
                   GUANDAO_P1_DECEL_UNITS_PER_S);
           portion1_speed_last_ms = speed_now_ms;
       }
       else
       {
           v_center = portion1_speed_planner.command;
       }
   }
   guandao_debug_speed_command = v_center;
...
void guandao_recode(guandao_state * state)
{
    ...
    if(portion3_save_pending)
    {
        ...
        uint8 reverse_ready;
        int16 direct_route_length;
        state_t direct_current_state;
        guandao_record_current_endpoint(p);
        direct_route_length = p->length_index;
        direct_current_state = p->current_state;
        for(int16 i = 0; i < direct_route_length; i++)
        {
            guandao_planned_map[i] = p->recode_map[i];
        }
        Flash_Store_Mode(route_setting_choice);
        p->length_index = direct_route_length;
        p->current_state = direct_current_state;
        for(int16 i = 0; i < direct_route_length; i++)
        {
            p->recode_map[i] = guandao_planned_map[i];
        }
        reverse_ready = portion3_direct_reverse_prepare(p);
        if(!reverse_ready)
        {
            rear_motor_brake_start();
        }
...
//    static uint8 flag2 = 1;
    ...
    if(portion3_direct_reverse_active())
    {
        portion3_direct_reverse_update();
        follow_points_show(&portion_3);
        return;
    }
